...
\cite{minamoto2026}, which formalizes symbol grounding as a
...
In a companion paper \cite{minamoto2026}, we proposed a structural
...
framework from \cite{minamoto2026}. Section~3 argues that fixed
...
We briefly summarize the framework of \cite{minamoto2026}, to which we
refer the reader for full definitions and proofs.

\begin{definition}[Sensory Space \cite{minamoto2026}]
...
\begin{definition}[Grounding Map \cite{minamoto2026}]
...
\begin{definition}[Grounding-Induced Topology \cite{minamoto2026}]
...
The key result of \cite{minamoto2026} is that homeomorphism of
grounding-induced topologies is \emph{sufficient} for complete intensional
concept sharing (Conjecture~1 of \cite{minamoto2026}), while topological
...
\cite{minamoto2026}), while CGLA computational graphs are finite and
...
condition of \cite{minamoto2026}---that the symbolic layer's induced
...
\cite{minamoto2026}, fixed-dataflow architectures incur a strictly
positive $d_{\mathrm{cog}}$ lower bound for sensory spaces not
